...
def get_nodes(port):
    """
    Se conecta al nodo inicial (en el puerto especificado) para obtener la lista de nodos.
    """
...
def get_node_info(port):
    """
    Se conecta al nodo (en el puerto especificado) para obtener la información del nodo.
    """
    event = random.randint(1, int(1e9))
        ...
        for node in nodes:
            try:
                if isinstance(node, dict):  # Extraer IP y puerto
                    node_ip = node["ip"]
                    node_port = BASE_PORT + int(node_ip.split(".")[-1])  # 65440 + último octeto
                else:
...
def refresh():
    """
    Ruta para refrescar la información usando el puerto del nodo inicial.
    """
    try:
        port = int(request.form.get('port'))
    except ValueError:
